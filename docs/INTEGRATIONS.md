...
|---|---|---|---|
| **AI image-gen** — Nano Banana / Gemini, fal.ai, Replicate Flux (`skills/hero-art`) | Bespoke hero art, seamless textures, OG images | **Paid** API / usage-billed (key) | `claude mcp add` the provider's image MCP with the key. **Free fallback:** shaders/CSS/3D (`wow`) + Pexels |
| **Pexels API** (`skills/hero-art`) | Real stock photography | **Free** (key) | `PEXELS_API_KEY` |
...
| **poly.pizza API** (`skills/3d-assets`) | Low-poly 3D models | **Free** hobby / PAYG commercial (key) | API key. **CC0 alt:** Poly Haven (keyless) |
